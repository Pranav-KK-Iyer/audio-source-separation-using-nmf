...
x=x(:);%make sure its a column
M=length(window);
if(M<2)error(...
        'myspectrogram: Expect complete window,now just its length');
end

if length(x)<M%zero-pad to fill a window:
    x=[x;zeroes(M-length(x),1)];
end
Modd=mod(M,2);%0 if M even 1 if M odd
Mo2=(M-Modd)/2;
...
for m=1:nframes
    ...
    X(:,m)=fft(xwzp);
    xoff=xoff+nhop;%advancein[ut offset byt hopsize
end
if(nargout==0)|doplot
    ...
    xdb=20*log10(abs(X));
    Xmax=max(max(Xdb));
    %clip lower llimit to -dbdown dB so nulls dont dominate
